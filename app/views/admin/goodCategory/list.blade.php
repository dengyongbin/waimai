            ...
            <div>
                ...
                <div class="widget-body no-padding">
                    ...
                    <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                        ...
                        @foreach ($goodCategorys as $key => $goodCategory)
                        <tr>
                            <td>{{{ $key + 1 }}}</td>
                            <td>{{{ $goodCategory->id }}}</td>
                            <td>{{{ $goodCategory->name }}}</td>
                            <td>{{{ $goodCategory->sort }}}</td>
                            <td>{{{ $goodCategory->created_at }}}</td>
                            <td>
                                <button id="editBtn" type="button" onclick="location='{{{ URL::to('admin/goodCategory/edit/'.$goodCategory->id) }}}';">编辑</button>
                                <button id="deleteBtn" type="button" value="{{{ $goodCategory->id }}}">删除</button>
                            </td>
                        </tr>
                        @endforeach
